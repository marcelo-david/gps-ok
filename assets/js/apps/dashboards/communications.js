	...
	$.ajax({
		...
		success: function (response) {
			...
			$.each( response, function( key, value ) {
				...
				$("#imgMapDevice").css('background-image','url(https://maps.googleapis.com/maps/api/staticmap?key='+sessionStorage.getItem("googlekey")+'&zoom=17&size=950x320&maptype=hybrid&markers=icon:'+img+'|'+value.latitude+','+value.longitude+"&sensor=false&scale=1)");
				$("#btnMapDevice").click();
			});												
		}
	});				

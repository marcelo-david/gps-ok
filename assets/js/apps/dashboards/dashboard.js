	...
	var streetview = null;
	var pano_device = 0;
    //Maps por default en CDMX
	var fenway = new google.maps.LatLng(19.4265802,-99.1335253,13);
...
$('.datetimepicker').bootstrapMaterialDatePicker({
    ...
    mymap = L.map('mapid', {
		...
		var googleRoad = L.tileLayer('https://mt0.google.com/vt/lyrs=m,traffic&hl=en&x={x}&y={y}&z={z}&s=Ga'
		...
		var googleHybrid = L.tileLayer('https://mt0.google.com/vt/lyrs=y,traffic&hl=pt_en&x={x}&y={y}&z={z}&s=Ga'
			/*('https://api.tiles.mapbox.com/v4/{id}/{z}/{x}/{y}.png?access_token=pk.eyJ1IjoiYXJvdmVudCIsImEiOiJjank1ZjlyeHEwNnloM2RvMDBxYm1mM2EyIn0.fZv98GVtRj6dylmX6ZGqZQ')*/, {
				attribution: 'Map data &copy; <a href="http://openstreetmap.org">OpenStreetMap</a> contributors, ' +
					'<a href="http://creativecommons.org/licenses/by-sa/2.0/">CC-BY-SA</a>, ' +
					'Imagery Â© <a href="http://mapbox.com">Mapbox</a>',
		...
		// var roadMutant = L.tileLayer('https://mt0.google.com/vt/lyrs=m,traffic&hl=en&x={x}&y={y}&z={z}&s=Ga'
		// 	/*('https://api.tiles.mapbox.com/v4/{id}/{z}/{x}/{y}.png?access_token=pk.eyJ1IjoiYXJvdmVudCIsImEiOiJjank1ZjlyeHEwNnloM2RvMDBxYm1mM2EyIn0.fZv98GVtRj6dylmX6ZGqZQ')*/, {
		// 		attribution: 'Map data &copy; <a href="http://openstreetmap.org">OpenStreetMap</a> contributors, ' +
		// 			'<a href="http://creativecommons.org/licenses/by-sa/2.0/">CC-BY-SA</a>, ' +
		// 			'Imagery Â© <a href="http://mapbox.com">Mapbox</a>',
		...
		// 		id: 'mapbox.streets'
		// 	}).addTo(mymap);
        //Mapa central en CDMX
            mymap.setView([19.4265802,-99.1335253,13], 8);
	...
	/*	L.Control.Watermark = L.Control.extend({
		  ...
		  $("#list_info_search").hide('');
           ...
           $("#list_alarms").show('');
			...
			if ($("#list_info:first").is(":hidden")) {
				...
				banderaAlertaListado = false;						
				//doListEvents();
				// setTimeout(function() {
				    doUpdateListEvents2(_.sortByOrder(events, ['id'], ['desc']));
				//  },1000);
				...
				$("#list_info").slideToggle("slow", function () {
					...
					if (banderaAlertaListado == false) {
						banderaAlertaListado = true;
					}
        ...
        //Agregar Geocercas desde mapa
         $('#geo').on('click', function () {
         //Oculta el panel de dispositivos para que no afecte en el play back
         $("#list_info").hide();
       	 //$("#formularioGeo").click();
          doOpenAlertError("Geofences button coming soon");
         //alert("Boton de Geocercas")
		});
        
        //Agregar conductores desde mapa
         $('#rfid').on('click', function () {
         //Oculta el panel de dispositivos para que no afecte en el play back
         $("#list_info").hide();
       	 $("#formulario_rfid").click();
          doOpenAlertError("RFID button coming soon");
        // alert("Boton de RFID")
		});
		...
		$('#btnSupport').on('click', function () {
			$(location).attr('href', "http://api.whatsapp.com/send?1=pt_EN&phone=917010022101");
		});
       ...
       $('#device_close_arriba').on('click', function () {
		...
		$("#viewInfoDevice").on("show.bs.modal", function () {
                    $('#list_info').hide();
                })
			...
			success: function (response) {
				...
				$.each(response, function (key, value) {
					...
					devices.push({
                        ...
                        input3: ((value.attributes.input3 == null || value.attributes.input3 == "") ? "input3" : value.attributes.input3),
						input4: ((value.attributes.input4 == null || value.attributes.input4 == "") ? "input4" : value.attributes.input4),
                        
                        output1: ((value.attributes.output1 == null || value.attributes.output1 == "") ? "output1" : value.attributes.output1),
						output2: ((value.attributes.output2 == null || value.attributes.output2 == "") ? "output2" : value.attributes.output2),
                        output3: ((value.attributes.output3 == null || value.attributes.output3 == "") ? "output3" : value.attributes.output3),
						output4: ((value.attributes.output4 == null || value.attributes.output4 == "") ? "output4" : value.attributes.output4),
						alarm: null,
                        output1Active: null,
                        output2Active: null,
                        output3Active: null,
	...
	function consultaWeb(arr,res){
		var tipoNombre = arr.attributes.alarm;
		var bandera;
					
		$.each(res, function (key, value) {					
			var attributesAlarm = value.attributes.alarms;
			var tempAttributesAlarm = "";
			var objIndex = _.findIndex(events, function(event) {
					return event.id == arr.id;
				});

			if (value.notificators == "web") {
				if(value.type == "alarm" && value.type != "deviceUnknown"){
						//console.log(tipoNombre);
						tempAttributesAlarm = attributesAlarm.split(/&amp|;/);
						//console.log(tipoNombre + "tipNombre === " + tempAttributesAlarm);
						if (tempAttributesAlarm.indexOf(tipoNombre) != -1  && attributesAlarm == "sos" || tempAttributesAlarm.indexOf(tipoNombre) != -1 && attributesAlarm == "door") {
							bandera = true;
						}else if (tempAttributesAlarm.indexOf(tipoNombre) != -1){
							bandera = true;
						}else{
							bandera = false;
						}							
						
					}else if (value.type.indexOf(arr.type) != -1 && value.type != "alarm" && value.type != "deviceUnknown") {
						bandera = true;
					}else {
						bandera = false;
					}					

				if (objIndex === -1 && bandera === true ) {
					events.push({
						id: arr.id,
						attributes: {
							alarm:  tipoNombre  
						...
						deviceName: findDevice(arr.deviceId).name,
						serverTime: arr.serverTime,
						type: arr.type,
						typeName: findEventType(arr.attributes.alarm ? arr.type + _.capitalize(arr.attributes.alarm) : arr.type).name
					});
	...
	function updateAlerts(response){
		...
		$.each(lastEvents, function (key, arr) {
			console.log(arr);
			...
			console.log();
			if (!jQuery.isEmptyObject(value.attributes)){
		...
		$.ajax({
			...
			success: function (response) {
				...
				$.each(response, function (key, value) {
                    ...
                    var input3Active = (value.attributes.input3 == null ? false : value.attributes.input3);
					var input4Active = (value.attributes.input4 == null ? false : value.attributes.input4);
                    var output1Active = (value.attributes.output1 == null ? false : value.attributes.output1);
					var output2Active = (value.attributes.output2 == null ? false : value.attributes.output2);
                    var output3Active = (value.attributes.output3 == null ? false : value.attributes.output3);
					var output4Active = (value.attributes.output4 == null ? false : value.attributes.output4);
					// markers
					var marker = L.marker([value.latitude, value.longitude], {
						icon: getIcon(dispositivo.category, dispositivo.name, convertSpeed(value.speed, 'knots'), communication, alarm, (input1Active || input2Active),(input3Active || input4Active),(output1Active || output2Active),(output3Active || output4Active), value.deviceId, ('ignition' in value.attributes) ? value.attributes.ignition : "none")
					}).on('click', function (e) {
                ...
                $("#countOfflinea").html("Offline (" + countOffline + ")");
                $("#countOnlinea").html("Online (" + countOnline + ")");
...
//Footer al click en maker en mapa
		...
		$.ajax({
			...
			success: function (response) {
				...
				$.each(response, function (key, value) {
					...
					var html =             
						"<div style='font-weight:bold'>" + $.i18n("title_protocol") + "&nbsp;:&nbsp;" + value.protocol + '</div>' +
						"<div style='font-weight:bold'>" + $.i18n("title_identifier") + "&nbsp;:&nbsp;" + dispositivo.uniqueId + '</div>' +
                        "<div style='font-weight:bold'>" + $.i18n("title_phone") + "&nbsp;:&nbsp;" + (dispositivo.phone == null ? '-' : dispositivo.phone) + '</div>' +                        
						"<div style='font-weight:bold'>" + $.i18n("title_model") + "&nbsp;:&nbsp;" + (dispositivo.model == null ? '-' : dispositivo.model) + '</div>' +
						"<div style='font-weight:bold'>" + $.i18n("title_category") + "&nbsp;:&nbsp;" + $.i18n("title_type_" + dispositivo.category) + '</div>' +
						"<div style='font-weight:bold'>" + $.i18n("title_device_time") + "&nbsp;:&nbsp;" + moment(value.deviceTime).format('DD/MM/YYYY HH:mm:ss') + '</div><br />' +
						"<div style='font-weight:bold'>" + $.i18n("title_coordinates") + "&nbsp;:&nbsp;" + value.latitude.toFixed(6) + ', ' + value.longitude.toFixed(6) + '</div>' +
						"<div style='font-weight:bold'>" + $.i18n("title_address") + "&nbsp;:&nbsp; <a href='https://www.google.com/maps/place/" + value.latitude + "," + value.longitude + "/@" + value.latitude + "," + value.longitude + ",17z/data=!3m1!4b1!4m5!3m4!1s0x0:0x0!8m2!3d" + value.latitude + "!4d" + value.longitude + "' target='_blank'>" + value.address + " <img src='assets/img/icons/navigation.png' style='width:16px;height:16px' /></a></div>" +
						"<div style='font-weight:bold'>" + $.i18n("title_speed") + "&nbsp;:&nbsp;" + convertSpeed(value.speed, 'knots') + findSpeedUnit(sessionStorage.getItem('speedUnit')).title + '</div>' +
                    ...
                    $("#deviceInput3").html($.i18n("title_" + dispositivo.input3));
					$("#deviceInput4").html($.i18n("title_" + dispositivo.input4));
                    $("#deviceOutput1").html($.i18n("title_" + dispositivo.output1));
					$("#deviceOutput2").html($.i18n("title_" + dispositivo.output2));
                    $("#deviceOutput3").html($.i18n("title_" + dispositivo.output3));
					$("#deviceOutput4").html($.i18n("title_" + dispositivo.output4));
					var input1Active = (value.attributes.input1 == null ? false : value.attributes.input1);
					var input2Active = (value.attributes.input2 == null ? false : value.attributes.input2);
                    var input3Active = (value.attributes.input3 == null ? false : value.attributes.input3);
					var input4Active = (value.attributes.input4 == null ? false : value.attributes.input4);
				...
				//	}
					...
					if (input2Active == true) {
                    	...
                    	if (input3Active == true) {
						$("#deviceInput3").addClass('label-success').removeClass('label-default');
					} else {
						$("#deviceInput3").addClass('label-default').removeClass('label-success');
					}
                    	if (input4Active == true) {
						$("#deviceInput4").addClass('label-success').removeClass('label-default');
					} else {
						$("#deviceInput4").addClass('label-default').removeClass('label-success');
					}
                    if (output1Active == true) {
						$("#deviceOutput1").addClass('label-success').removeClass('label-default');
					} else {
						$("#deviceOutput1").addClass('label-default').removeClass('label-success');
					}
                    if (output2Active == true) {
						$("#deviceOutput2").addClass('label-success').removeClass('label-default');
					} else {
						$("#deviceOutput2").addClass('label-default').removeClass('label-success');
					}
                    if (output3Active == true) {
						$("#deviceOutput3").addClass('label-success').removeClass('label-default');
					} else {
						$("#deviceOutput3").addClass('label-default').removeClass('label-success');
					}
                    if (output4Active == true) {
						$("#deviceOutput4").addClass('label-success').removeClass('label-default');
					} else {
						$("#deviceOutput4").addClass('label-default').removeClass('label-success');
					}
	...
	function panTo(deviceId, openInfo) {
	 ...
	 var objIndex = devices.findIndex((obj => obj.deviceId == deviceId));
        ...
        if (devices[objIndex].latitude == null) {
	   //doOpenAlertError("El dispositivo aun no esta reportando a la plataforma GPS");
		} else {
         //doOpenAlertError("El dispositivo si tiene lat");
	...
	function doUpdateListDevices(arr) {
		...
		$.each(_groups, function (key, group) {
			...
			$.each(group, function (key, value) {
				...
				var ignitionTitle = (value.ignition == "none" ? "ACC not available" : (value.ignition == true ? "ACC ON" : "ACC OFF"));
				...
				var movementTitle = (value.speed > 5 ? "Moving" : "Stopped");
				...
				//var candadoTitle = (value.candado == "none" ? "State no disponible" : (value.candado == true ? "Bloqueado" : "Desbloqueado"));
			...
			error: function (response) {
				switch (response.status) {
					case 400:
						...
						doOpenAlertInfo((/dispositivo is not online/i.test(response.responseText)) ? $.i18n("message_device_not_online") : $.i18n("message_user_not_permission"));
						break;
					case 401:
		...
		$.ajax({
			...
			success: function (response) {
					...
					switch ($("#report_id").val()) {
						...
						case "0":
							...
							var autonomy = dispositivo.autonomy;
                             var devicestempHoras = msToTime(value.engineHours);  

							if (autonomy > 0) {
								var fuelCost = (((value.Distância / 1000) / autonomy) * parseFloat(sessionStorage.getItem(fuel)));
							}

							dataSetReport.push([value.deviceName,
                            convertMeters(value.Distância,sessionStorage.getItem('distanceUnit')),
                            //hhmmss(value.engineHours, 'milliseconds'),      
                            horas = devicestempHoras,
                            convertSpeed(value.maxSpeed, 'knots').toFixed(2),
                            //convertSpeed(value.averageSpeed, 'knots'),
                            convertSpeed(value.averageSpeed, 'knots').toFixed(2),
                            parseFloat(fuelCost).toFixed(2)]);
							break;
						case "1":
                                                    //console.log("************");
                                                    //console.log("PASO 1");
                                                    //console.log("************");
							dataSetReport.push([
                                                            moment(value.serverTime).format('DD/MM/YYYY HH:mm:ss')
                                                            , findDevice(value.deviceId).name
                                                            , findEventType(value.attributes.alarm ? value.type + _.capitalize(value.attributes.alarm) : value.type).name
                                                            , (value.geofenceId > 0 ? findGeofence(value.geofenceId).name : "")
                                                        ]);
                                                    ///console.log(dataSetReport);
                                                    //console.log("************");
                                                    //console.log("PASO 2");
						...
						case "2":
                            ...
                            value.startAddress, moment(value.endTime).format('DD/MM/YYYY HH:mm:ss'),
                            value.endAddress, convertMeters(value.Distância, "km").toFixed(2),
                            convertSpeed(value.averageSpeed, 'knots').toFixed(2),
						...
						case "3":
                            ...
                            value.valid,
                            moment(value.deviceTime).format('DD/MM/YYYY'),moment(value.deviceTime).format('HH:mm:ss'),
                            value.latitude,
                            value.longitude,
                            convertSpeed(value.speed, 'knots').toFixed(2),
                            findEventType(value.attributes.alarm ? 'alarm' + _.capitalize(value.attributes.alarm) : 'alarm').name,
                            degToCompass(value.course), value.address]);
							break;
						case "4":
							dataSetReport.push([value.deviceName,
								moment(value.startTime).format('DD/MM/YYYY HH:mm:ss'),
						...
						case "5":
                            ...
                            var input3Active = (value.attributes.input3);
							var input4Active = (value.attributes.input4);
                            
                            var output1Active = (value.attributes.output1);
							var output2Active = (value.attributes.output2);
                            var output3Active = (value.attributes.output3);
							var output4Active = (value.attributes.output4);


							if (input1Active) {
								var dispositivo = findDevice(value.deviceId);
								dataSetReport.push([findDevice(value.deviceId).name, moment(value.deviceTime).format('DD/MM/YYYY HH:mm:ss'), value.address, (value.attributes.input1? $.i18n("title_on") : $.i18n("title_off")), convertSpeed(value.speed, 'knots')+ ' ' +speedUnit, $.i18n("title_" + dispositivo.input1)]);
							}

							if (input2Active) {
								var dispositivo = findDevice(value.deviceId);
								dataSetReport.push([findDevice(value.deviceId).name, moment(value.deviceTime).format('DD/MM/YYYY HH:mm:ss'), value.address, (value.attributes.input2? $.i18n("title_on") : $.i18n("title_off")), convertSpeed(value.speed, 'knots')+ ' ' +speedUnit, $.i18n("title_" + dispositivo.input2)]);
							}
                            
                            if (input3Active) {
								var dispositivo = findDevice(value.deviceId);
								dataSetReport.push([findDevice(value.deviceId).name, moment(value.deviceTime).format('DD/MM/YYYY HH:mm:ss'), value.address, (value.attributes.input3? $.i18n("title_on") : $.i18n("title_off")), convertSpeed(value.speed, 'knots')+ ' ' +speedUnit, $.i18n("title_" + dispositivo.input3)]);
							}
                            
                            if (input4Active) {
								var dispositivo = findDevice(value.deviceId);
								dataSetReport.push([findDevice(value.deviceId).name, moment(value.deviceTime).format('DD/MM/YYYY HH:mm:ss'), value.address, (value.attributes.input4? $.i18n("title_on") : $.i18n("title_off")), convertSpeed(value.speed, 'knots')+ ' ' +speedUnit, $.i18n("title_" + dispositivo.input4)]);
							}
                            
                            
                            if (output1Active) {
								var dispositivo = findDevice(value.deviceId);
								dataSetReport.push([findDevice(value.deviceId).name, moment(value.deviceTime).format('DD/MM/YYYY HH:mm:ss'), value.address, (value.attributes.output1? $.i18n("title_on") : $.i18n("title_off")), convertSpeed(value.speed, 'knots')+ ' ' +speedUnit, $.i18n("title_" + dispositivo.output1)]);
							}
                            if (output2Active) {
								var dispositivo = findDevice(value.deviceId);
								dataSetReport.push([findDevice(value.deviceId).name, moment(value.deviceTime).format('DD/MM/YYYY HH:mm:ss'), value.address, (value.attributes.output2? $.i18n("title_on") : $.i18n("title_off")), convertSpeed(value.speed, 'knots')+ ' ' +speedUnit, $.i18n("title_" + dispositivo.output2)]);
							}
                            if (output3Active) {
								var dispositivo = findDevice(value.deviceId);
								dataSetReport.push([findDevice(value.deviceId).name, moment(value.deviceTime).format('DD/MM/YYYY HH:mm:ss'), value.address, (value.attributes.output3? $.i18n("title_on") : $.i18n("title_off")), convertSpeed(value.speed, 'knots')+ ' ' +speedUnit, $.i18n("title_" + dispositivo.output3)]);
							}
                            if (output4Active) {
                                ...
                                value.address, (value.attributes.output4? $.i18n("title_on") : $.i18n("title_off")),
                                convertSpeed(value.speed, 'knots')+ ' ' +speedUnit,
                                $.i18n("title_" + dispositivo.output4)]);
							}
							break;
						case "6":
				...
				switch ($("#report_id").val()) {
					...
					case "0":
						$("#Cabeçalho do realatório").append('<tr>' +
						...
						title_report = $.i18n("Resumo dos reatórios");
						break;
					case "1":
						$("#Cabeçalho do realatório").append('<tr>' +
							'<th>Data / hora</th>' +
							...
							'<th>Geocerca</th>' +
							'</tr>');
						title_report = $.i18n("Relatórios de eventos");
						break;
					case "2":
						$("#Cabeçalho do realatório").append('<tr>' +
...
function formCommandClear() {
	...
	function plotMarkers() {
		...
		$.each(devices, function (key, value) {
			...
			// markers
			var marker = L.marker([value.latitude, value.longitude], {
				icon: getIcon(value.category, value.name, convertSpeed(value.speed, 'knots'), value.communication, value.alarm, (value.input1Active || value.input2Active),(value.input3Active || value.input4Active),(value.output1Active || value.output2Active),(value.output3Active || value.output4Active), value.deviceId, value.ignition)
			}).on('click', function (e) {
...
function IniciarConexion(){
    ...
    //         var i, j, store, data, array, entity, dispositivo, typeKey, alarmKey, text, geofence;
    ...
    //         //doLastPositions2();
    //         //doOpenAlertError("WEb socket cerrado hacer algo");
    //         	$(location).attr('href', "dashboard.php");
    //
    //     };
    // });
	var ws = new WebSocket("ws://178.128.107.147:8082/api/socket");
...
function doLastPositions2(response) {
    ...
    function doUpdateListEvents2(arr) {

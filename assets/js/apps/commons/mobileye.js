...
function doPrepareEvents() {
    ...
    console.log(sessionStorage.getItem('url') + "reports/events")
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            $.ajax({
                ...
                success: function(pocisiones) {  

                    //console.log("positions = "+ dump(response));
                    ...
                    if (j != -1) {
                     ...
                     n = parameters.search("allEvent")
                    if (n != -1) {
                        parameters = "lock"
                    }
                   //console.log(parameters)
                    $.each(response, function(key, value) {
                        var alarm = ('alarm' in value.attributes) ? value.attributes.alarm : "";
                        var positionInf = findPositionId(pocisiones, value.positionId);
                        console.log(dump(positionInf));
                        var deviceIdTemp = findDevice(value.deviceId).name;
                        newalarm = alarm.toUpperCase()
                        parameters = parameters.toUpperCase()
                        if (j != -1) {
                            n = newalarm.search(parameters);
                        } else {
                            n = parameters.search(newalarm);
                        }
                        //console.log(parameters)
                        //console.log(newalarm)
                        //console.log(n)
                        if (j != -1) {
                            console.log('la alarma=' + alarm)

                            alarm = traduce(alarm);
                         
                            //console.log(dump(value));
                           
                                // var tipo = ('tipo' in value.attributes) ? value.attributes.tipo : "";
                           // dataSet.push([moment(value.serverTime).format('DD/MM/YYYY HH:mm:ss'), findDevice(value.deviceId).name, traduce(alarm), value.speed]);
                            dataSet.push([moment(value.serverTime).format('DD/MM/YYYY HH:mm:ss'), deviceIdTemp, alarm, positionInf.latitude, positionInf.longitude, (positionInf.address == null ? "" : positionInf.address),(positionInf.speed == 0 ? "" : positionInf.speed)]);
                            if (names.indexOf(alarm) == -1) {
                                names.push(alarm);
                                //marcadores.push();                                                     
                            }                    
                         ...
                         , 
                         {
                            ...
                            customize: function(win) {
                                var header = '<span style="position:absolute;top:53px;left:0;">' + $.i18n('title_period') + ':' + $("#from").val() + " => " + $("#to").val() + '</span>' +
                                    ...
                                    //'<img src="https://smartgps.mx/plataforma/assets/img/logo.jpg" style="position:absolute; top:0; right:0; width:110; height:70; " />';
                                ...
                                $(win.document.body)
                                    .append(                                
                                        '<img src="'+imageData+'" style="position:static; right: 0%; width: 90%; height:90%; " />'
                                    );                            
...
function traduce(alarma) {
    ...
    switch (alarma) {
        ...
        case 'mobileyeTamper':
            final = 'Mobileye Tamper Activado'
            break;
        case 'mobileyeLr':
            final = 'Detector de Lineas Viales Derecha LDW'
            break;
        case 'mobileyePcw':
            final = 'Advertencia de Colisi\u00F3n a Peatones. PCW'
            break;
        case 'mobileyeUfcw':
            final = 'Advertencia Urbana de Colisi\u00F3n Frontal por debajo de 30 km/h. UFCW'
            break;
        case 'mobileyeError':
            final = 'Mobileye Error'
            break;
        case 'mobileyeTsr':
            final = 'Indicador de Limite de Velocidad Excedida. SLI'
            break;
        case 'mobileyeHmw':
            final = 'Advertencia de Distancia Segura Frontal. HMW'
            break;
        case 'mobileyeLl':
            final = 'Detector de lineas Viales Izquierda LDW'
            break;
        case 'MobileyeFcw':
            final = 'Advertencia de Colision Frontal. FCW'
            break;
            
            case 'lockRing':
            final = 'I-Lock Anillo cerrado'
            break;
            case 'lockClosed':
            final = 'I-Lock Candado Cerrado'
            break;
            case 'lockOpen':
            final = 'I-Lock Candado abierto'
            break;
            
            case 'sos':
            final = 'I-Lock Alerta de p\u00E1nico'
            break;
            
            case 'lowBattery':
            final = 'I-Lock Bater\u00EDa baja'
            break;
...
function grafica(nombres, namDevice){
    ...
    var options = {
        ...
        title: {
            text: 'Reporte de Eventos Mobileye'
        },
        subtitle: {
            text: 'Listado de eventos de dispositivos Iwatcher con integraci\u00F3n de Mobileye'
        },
        accessibility: {
            announceNewData: {
                enabled: true
    ...
    /*let svgString = char.getSVG();
    //console.log(svgString)

    // Use DOMParser to parse new svg element from svgString
    let parser = new DOMParser(); 
    let svgElem = parser.parseFromString(svgString, "image/jpeg").documentElement;
    //console.log(svgElem);

        ...
        success: function (response) {
            ...
            $.each(response, function (key, value) {
                ...
                if ($("#disabled").val() == 'all' || disabled == $("#disabled").val()) {                                        
                    ...
                    dataSet.push([value.id, deviceIdTemp, value.uniqueId, groupName, $.i18n("title_type_"+value.category), value.model, value.phone, $.i18n("title_"+disabled)]);
                if (names.indexOf($.i18n("title_"+disabled)) == -1) {
                                names.push($.i18n("title_"+disabled));
                                //marcadores.push();                                                     
                            }  
                }
            });
...
function grafica(nombres, namDevice){
    ...
    var options = {
        ...
        title: {
            text: 'Report disabled users'
        },
        subtitle: {
            text: 'User status list'
        },
        accessibility: {
            announceNewData: {
                enabled: true

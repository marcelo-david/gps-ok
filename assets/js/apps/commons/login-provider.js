    ...
    $.ajax({
            ...
            case 400:
               ...
               // break;
                mensaje = response.responseText;
                    ...
                    if(validacion[0] == "Account"){
                        doOpenAlertError("EL usuario se encuentra desactivado del sistema");
                    }else{
                         doOpenAlertError((/Account has expired /i.test(response.responseText)) ? $.i18n("account_has_expired") : $.i18n("message_user_not_permission"));
                    }
                     break;
        ...
        'success': function (data, status, xhr) {
            if (!data.disabled) {
                ...
                //sessionStorage.setItem('admin', !('admin' in data) ? $.parseJSON(data.administrator) ? '1' : '0' : $.parseJSON(data.admin) ? '1' : '0');
                ...
                sessionStorage.setItem('expireTime', moment()['add']('minutes', 15));
                ...
                sessionStorage.setItem('manager', data.userLimit == 0 ? '0' : '1');
                ...
                sessionStorage.setItem('providerId', !('providerId' in data.attributes) ? '' : data.attributes.providerId);
...
function saveUserSettings(settings) {
    ...
    data.providerId = sessionStorage.getItem('providerId');
    ...
    var home_page1 = '../../dashboard_provider.php';
    switch (sessionStorage.getItem('panel')) {
    case 'communication':
        home_page1 = '../../main_provider.php';
        break;
    default:
        home_page1 = '../../dashboard_provider.php';
        break;
    }
    $.ajax({
        'type': 'PUT',
...
function configurationServer() {
    ...
    $.ajax({
        ...
        'cache': ![],
        'error': function (response) {
            doOpenAlertError('Error al realizar la operación, contacte a soport tecnico:' + response.statusText);
        },
        'success': function (data) {
            var data = eval('(' + data + ')');
...
function gpsServer() {
    $.ajax({
        ...
        'success': function (data) {
            ...
            var page_title = 'Gennissi Tracking System 2019 Provider';

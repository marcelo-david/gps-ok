    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            $.each(response, function(key, value) {
                conManteDriverGeof(value, value.id);               
            });
...
function conManteDriverGeof(value, id){
    if (typeof id != 'undefined') {
                    // Javascript sourced data
                    $.ajax({
                        type : "get",
                        url : sessionStorage.getItem('url') + "drivers?deviceId="+id,
                        //data:params,    
                        cache: false,       
                        headers: {
                            "Authorization": "Basic " + btoa(sessionStorage.getItem('email')+":"+sessionStorage.getItem('password'))
                        ...
                        success: function (respDrivers) {

                            $.ajax({
                                type : "get",
                                url : sessionStorage.getItem('url') + "maintenance?deviceId="+id,
                                //data:params,    
                                cache: false,       
                                headers: {
                                    "Authorization": "Basic " + btoa(sessionStorage.getItem('email')+":"+sessionStorage.getItem('password'))
                                ...
                                success: function (respMain) {
                                    $.ajax({
                                        type : "get",
                                        url : sessionStorage.getItem('url') + "geofences?deviceId="+id,
                                        //data:params,    
                                        cache: false,       
                                        headers: {
                                            "Authorization": "Basic " + btoa(sessionStorage.getItem('email')+":"+sessionStorage.getItem('password'))
                                        ...
                                        success: function (respGeo) {

                                            var status = value.status;
                                            //var status = $.i18n("title_" + value.status);
                                            //console.log($("#status").val() + ' vs ' + value.status)
                                            var driversID = findDriversId(respDrivers);

                                            var maintenanceID = findMantenanceId(respMain, value.id);

                                            var geoCercasID = findGeoCercasId(respGeo);
                                            
                                            
                                            console.log(driversID +"  :::  "+maintenanceID);
                                            var deviceIdTemp = value.name;
                                            var state = ($.i18n("title_"+status));
                                            var ultimo = moment(value.lastUpdate).format('DD-MM-YYYY  HH:mm:ss a');
                                            
                                            //          $.i18n("title_"+status)
                                            if ($("#status").val() == 'all' || status == $("#status").val()) {

                                                dataSet.push([value.id, deviceIdTemp, ultimo, geoCercasID,driversID, maintenanceID, state]);
...
function findDriversId(driversTemp) {
    var temdrivers = [];
    var tempD = "";

    $.each(driversTemp, function(key, value){
        temdrivers.push((value.name ? value.name:value.uniqueId));
    });

    tempD = temdrivers.toString();
...
function findMantenanceId(maintenanceTemp, id) {
    ...
    $.each(maintenanceTemp, function(key, value){
        temM.push((value.name ? value.name:id));
    });
...
function findGeoCercasId(geoCTemp){
    var temG = [];
    var tempG = "";

    $.each(geoCTemp, function(key, value){
        temG.push((value.name ? value.name:""));
    });

    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            $.each(response, function(key, value) {
                ...
                html += '    </ul>';
                html += '</div>';
                // var alarms = $.i18n("title_"+value.alarms);
                var alarms = ("alarms" in value.attributes ? value.attributes.alarms : '');
                console.log('alarms=' + alarms)

                var type = $.i18n("title_" + value.type);
                dataSet.push([value.id, type, value.notificators, $.i18n("title_"+alarms), html]);
...
function doSaveGeofenceDevices() {
    ...
    params.notificationId = form.notificationId;
...
function doDeleteGeofenceDevices(notificationId, deviceId) {
...
function getGeofencesGeofenceDevices() {
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            limpaSelect("notificationId");

            $.each(response, function(key, value) {
                var type = $.i18n("title_" + value.type);

                if (value.attributes.alarms) {
                    type = "Alarma " + $.i18n("title_"+value.attributes.alarms)
                }

                //console.log("alarm type" + type)
                insertOption("notificationId", value.id, type);
            });

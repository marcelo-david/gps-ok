	...
	$(document).ready(function () {
		...
		$('#btnDevices').on('click', function () {
			...
			if ($("#list_info:first").is(":hidden")) {
				$("#list_info").show("slow");
				$("#list_info_title").html("Vehículos");
		...
		$('#btnStreetView').on('click', function () {
			...
			if (currentDevice > 0) {
				...
				doOpenAlertError("Primero seleccione un Vehículos!");
			}
	...
	function doUpdateListDevices(arr) {
		...
		$.each(_groups, function (key, group) {
			...
			$.each(group, function (key, value) {
				...
				var ignitionTitle = (value.ignition == "none" ? "ACC no disponible" : (value.ignition == true ? "Ignición ON ACC" : "Ignición OFF ACC"));

				var statusImg = (value.status == "online" ? "status-on" : "status-off");
				var statusTitle = (value.status == "online" ? "En línea" : "Fuera de línea");
				...
				$.each(response, function (key, value) {
					...
					switch ($("#report_id").val()) {
						...
						case "0":
							...
							dataSetReport.push([value.deviceName, convertMeters(value.Distância, sessionStorage.getItem('distanceUnit')), hhmmss(value.engineHours, 'milliseconds'), convertSpeed(value.maxSpeed, 'knots'), convertSpeed(value.averageSpeed, 'knots'), parseFloat(fuelCost).toFixed(2)]);
							break;
						case "1":

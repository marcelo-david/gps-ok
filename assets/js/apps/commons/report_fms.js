var dataSet = [];
var char;
let imageData;

var server_attributes = [];

$(document).ready(function() {

    /**
     * Check status of session of user
     */
    checkSession();

    localFormClear();

    loadSwitchery();

    /**
     * parameters this page
     */

    //Datetimepicker plugin
    $('.datetimepicker').bootstrapMaterialDatePicker({
        format: 'DD/MM/YYYY HH:mm:ss',
        clearButton: true,
        weekStart: 1,
        //lang : (sessionStorage.getItem('language')=='br'?'pt-BR':sessionStorage.getItem('language'))
        lang: (sessionStorage.getItem('language') == 'en' ? 'en' : sessionStorage.getItem('language'))
    });


    $('#form_report').validate({
        errorElement: 'span',
        focusInvalid: false,
        icon: {
            valid: 'glyphicon glyphicon-ok',
            invalid: 'glyphicon glyphicon-remove',
            validating: 'glyphicon glyphicon-refresh'
        },
        rules: {
            deviceId: {
                required: true
            },
            type: {
                required: true
            },
            from: {
                required: true
            },
            to: {
                required: true
            }
        },
        errorClass: 'validation-error-label',
        successClass: 'validation-valid-label',
        validClass: "validation-valid-label",

        invalidHandler: function(event, validator) { //display error alert on form submit   
            $('.alert-error', $('.login-form')).show();
        },

        highlight: function(element) {
            $(element).closest('.form-group').addClass('has-error');
            $(element).closest('.select-search').addClass('border-warning');
        },
        unhighlight: function(element) {
            $(element).closest('.form-group').removeClass('has-error');
            $(element).closest('.select-search').removeClass('border-warning');
        },

        success: function(e) {
            $(e).closest('.form-group').removeClass('error').addClass('info');
            $(e).remove();
            // e.addClass("validation-valid-label").text("Sucesso.");
        },

        errorPlacement: function(error, element) {
            if (element.is(':checkbox') || element.is(':radio')) {
                var controls = element.closest('.controls');
                if (controls.find(':checkbox,:radio').length > 1) controls.append(error);
                else error.insertAfter(element.nextAll('.lbl:eq(0)').eq(0));
            } else if (element.is('.select2')) {
                error.insertAfter(element.siblings('[class*="select2-container"]:eq(0)'));
            } else if (element.is('.chzn-select')) {
                error.insertAfter(element.siblings('[class*="chzn-container"]:eq(0)'));
            }
            // Input group, styled file input
            else if (element.parent().hasClass('uploader') || element.parents().hasClass('input-group')) {
                error.appendTo(element.parent().parent());
            } else error.insertAfter(element);
        },

        submitHandler: function(form) {},
        invalidHandler: function(form) {}
    });
    
      $('#form_report2').validate({
        errorElement: 'span',
        focusInvalid: false,
        icon: {
            valid: 'glyphicon glyphicon-ok',
            invalid: 'glyphicon glyphicon-remove',
            validating: 'glyphicon glyphicon-refresh'
        },
        rules: {
            deviceId: {
                required: true
            },
            type: {
                required: true
            },
            from: {
                required: true
            },
            to: {
                required: true
            }
        },
        errorClass: 'validation-error-label',
        successClass: 'validation-valid-label',
        validClass: "validation-valid-label",

        invalidHandler: function(event, validator) { //display error alert on form submit   
            $('.alert-error', $('.login-form')).show();
        },

        highlight: function(element) {
            $(element).closest('.form-group').addClass('has-error');
            $(element).closest('.select-search').addClass('border-warning');
        },
        unhighlight: function(element) {
            $(element).closest('.form-group').removeClass('has-error');
            $(element).closest('.select-search').removeClass('border-warning');
        },

        success: function(e) {
            $(e).closest('.form-group').removeClass('error').addClass('info');
            $(e).remove();
            // e.addClass("validation-valid-label").text("Sucesso.");
        },

        errorPlacement: function(error, element) {
            if (element.is(':checkbox') || element.is(':radio')) {
                var controls = element.closest('.controls');
                if (controls.find(':checkbox,:radio').length > 1) controls.append(error);
                else error.insertAfter(element.nextAll('.lbl:eq(0)').eq(0));
            } else if (element.is('.select2')) {
                error.insertAfter(element.siblings('[class*="select2-container"]:eq(0)'));
            } else if (element.is('.chzn-select')) {
                error.insertAfter(element.siblings('[class*="chzn-container"]:eq(0)'));
            }
            // Input group, styled file input
            else if (element.parent().hasClass('uploader') || element.parents().hasClass('input-group')) {
                error.appendTo(element.parent().parent());
            } else error.insertAfter(element);
        },

        submitHandler: function(form) {},
        invalidHandler: function(form) {}
    });
    
      $('#form_report3').validate({
        errorElement: 'span',
        focusInvalid: false,
        icon: {
            valid: 'glyphicon glyphicon-ok',
            invalid: 'glyphicon glyphicon-remove',
            validating: 'glyphicon glyphicon-refresh'
        },
        rules: {
            deviceId: {
                required: true
            },
            type: {
                required: true
            },
            from: {
                required: true
            },
            to: {
                required: true
            }
        },
        errorClass: 'validation-error-label',
        successClass: 'validation-valid-label',
        validClass: "validation-valid-label",

        invalidHandler: function(event, validator) { //display error alert on form submit   
            $('.alert-error', $('.login-form')).show();
        },

        highlight: function(element) {
            $(element).closest('.form-group').addClass('has-error');
            $(element).closest('.select-search').addClass('border-warning');
        },
        unhighlight: function(element) {
            $(element).closest('.form-group').removeClass('has-error');
            $(element).closest('.select-search').removeClass('border-warning');
        },

        success: function(e) {
            $(e).closest('.form-group').removeClass('error').addClass('info');
            $(e).remove();
            // e.addClass("validation-valid-label").text("Sucesso.");
        },

        errorPlacement: function(error, element) {
            if (element.is(':checkbox') || element.is(':radio')) {
                var controls = element.closest('.controls');
                if (controls.find(':checkbox,:radio').length > 1) controls.append(error);
                else error.insertAfter(element.nextAll('.lbl:eq(0)').eq(0));
            } else if (element.is('.select2')) {
                error.insertAfter(element.siblings('[class*="select2-container"]:eq(0)'));
            } else if (element.is('.chzn-select')) {
                error.insertAfter(element.siblings('[class*="chzn-container"]:eq(0)'));
            }
            // Input group, styled file input
            else if (element.parent().hasClass('uploader') || element.parents().hasClass('input-group')) {
                error.appendTo(element.parent().parent());
            } else error.insertAfter(element);
        },

        submitHandler: function(form) {},
        invalidHandler: function(form) {}
    });

    $('button[name="btnGenerate"]').click(function() {
        if ($('#form_report').valid()) {

            swal({
                    title: $.i18n("title_generate_report"),
                    text: "",
                    type: "info",
                    showCancelButton: true,
                    confirmButtonClass: "btn-info",
                    confirmButtonText: "Si",
                    cancelButtonText: "No",
                    closeOnConfirm: false
                },
                function() {
                    doPrepareEvents();
                });
        };
    });
    
       $('button[name="btnGenerate2"]').click(function() {
        if ($('#form_report2').valid()) {

            swal({
                    title: $.i18n("title_generate_report"),
                    text: "",
                    type: "info",
                    showCancelButton: true,
                    confirmButtonClass: "btn-info",
                    confirmButtonText: "Si",
                    cancelButtonText: "No",
                    closeOnConfirm: false
                },
                function() {
                    doPrepareEvents2();
                });
        };
    });
    
       $('button[name="btnGenerate3"]').click(function() {
        if ($('#form_report3').valid()) {

            swal({
                    title: $.i18n("title_generate_report"),
                    text: "",
                    type: "info",
                    showCancelButton: true,
                    confirmButtonClass: "btn-info",
                    confirmButtonText: "Si",
                    cancelButtonText: "No",
                    closeOnConfirm: false
                },
                function() {
                    doPrepareEvents3();
                });
        };
    });

    $('button[name="btnClear"]').click(function() {
        localFormClear();
    });
    
        
        $('#fms2').on('click', function () {
             loadSwitchery();
		getDevicesEvents2();
        console.log("le dimos clic al boton de fms2")

		});
        
         $('#fms3').on('click', function () {			
		 loadSwitchery();
		getDevicesEvents3();
        console.log("le dimos clic al boton de fms3")

		});
        

    getDevicesEvents();
    

    getGroups();

    getGeofences();

});

/**
 * Carrega listagem de dados
 *
 * @return void
 */

function doPrepareEvents() {

    doOpenAlertWait($.i18n("title_wait"), $.i18n("title_wait_report"));

    var form = $("#form_report").serializeObject();
    var params = "";
    var parameters = "";
    //var params = new Object();
    //prepare list devices
    var deviceIds = $('#deviceId option:selected');
    $(deviceIds).each(function(index, deviceId) {
        if ($(this).val() != "") {
            params += "deviceId=" + $(this).val() + "&";
        }
    });

    //prepare list groups

    var groups = $('#groupId option:selected');
    $(groups).each(function(index, group) {
        if ($(this).val() != "") {
            params += "groupId=" + $(this).val() + "&";
        }
    });
    //params += "groupId=0&";

    //prepare list type events

    var types = $('#type option:selected');
    $(types).each(function(index, type) {
        if ($(this).val() != "") {
            parameters += $(this).val() + "|";
        }
    });
    //params += "type=alarm&";

    var data_ini = moment(form.from, "DD/MM/YYYY hh:mm:ss");
    var data_end = moment(form.to, "DD/MM/YYYY hh:mm:ss");

    params += "from=" + data_ini.tz("Etc/GMT+0").format().substr(0, 19) + ".000Z&";
    params += "to=" + data_end.tz("Etc/GMT+0").format().substr(0, 19) + ".000Z";

    console.log(sessionStorage.getItem('url') + "positions")

    $.ajax({
        type: "GET",
        url: sessionStorage.getItem('url') + "reports/route",
        data: params,
        contentType: "application/json",
        cache: false,
        headers: {
            "Authorization": "Basic " + btoa(sessionStorage.getItem('email') + ":" + sessionStorage.getItem('password')),
            "Accept": "application/json"
        },
        error: function(response) {
            switch (response.status) {
                case 400:
                    doOpenAlertError($.i18n("message_user_not_permission"));
                    break;
                case 401:
                    doOpenAlertError($.i18n("message_user_unauthorized"));
                    break;
                case 404:
                    doOpenAlertError($.i18n("message_user_no_matching"));
                    break;
                default:
                    doOpenAlertError($.i18n("message_error_performing"));
                    break;
            }
        },
        success: function(response) {

            // Javascript sourced data
            dataSet.splice(0);

            // var alarm = ("attributes" in value.attributes?value.attributes.attributes:'');
            // dataSet.push([moment(value.serverTime).format('DD/MM/YYYY HH:mm:ss'), findDevice(value.deviceId).name, findEventType(value.type).name, attributes, (value.geofenceId>0?findGeofence(value.geofenceId).name:"")]);
            //console.log(parameters)
            //parameters.indexOf(alarm);
            //j = parameters.search("allEvent")
            //console.log('j=' + j)
            //if (j != -1) {
            //    parameters = "obd"
            //}
            console.log('Entro al success')
            var tempRpm =[];
            var tempTemper = [];
            var tempCombus = [];
            var tempTime = [];
            $.each(response, function(key, value) {
                var obd = ('Tipo' in value.attributes) ? value.attributes.Tipo : "";
                //console.log('el artibuto=' + obd)
                if (obd == "FMS1") {
                    //console.log('los rpm=' + value.attributes.rpm)

                    var engine_fuel = ('engine_total_fuel_used' in value.attributes) ? value.attributes.engine_total_fuel_used : "";
                    var fuel_level= ('dd_fuel_level_1' in value.attributes) ? value.attributes.dd_fuel_level_1 : "";
                    var engine_speed = ('eec1_engine_speed' in value.attributes) ? value.attributes.eec1_engine_speed : "";
                    var hours_engine_total = ('hours_engine_total_hours_of_operation' in value.attributes) ? value.attributes.hours_engine_total_hours_of_operation : "";
                    // dataSet.push([moment(value.serverTime).format('DD/MM/YYYY HH:mm:ss'), findDevice(value.deviceId).name, traduce(alarm), value.speed]);
                    dataSet.push([moment(value.serverTime).format('DD/MM/YYYY HH:mm:ss'), findDevice(value.deviceId).name, engine_fuel, fuel_level, engine_speed,  hours_engine_total]);
                    //tempRpm.push(rpm);
                    //tempTemper.push(coolantTemp);
                    //tempCombus.push(fuel);
                    //tempTime.push(moment(value.serverTime).format('DD/MM/YYYY HH:mm:ss'));                    
                }
                

                
                
                
            });

            //grafica(tempRpm, tempTemper, tempCombus, tempTime);

            $('.datatable-js').dataTable().fnDestroy();

            $('.datatable-js').dataTable({
                data: dataSet,
                columnDefs: [],
                responsive: true,
                dom: 'Blfrtip',
                colReorder: true,
                buttons: [{
                
                //{extend: 'excelHtml5',
			              // title: $.i18n("title_reports_events")
			              //}
                          
                           extend: 'pdfHtml5',
                            title: $.i18n("title_report_fms1"),
                            className: 'expPDF',                    
                            customize: function(doc) {
                            doc['pageMargins'] = [10, 70, 10, 10];
                            //doc.pageMargins = [10,10,10,10];
                            doc['header'] = (function (page, pages) {
                                                        return {
                                                            columns: [
                                                               {
                                                                    //Milipol
                                                                    /*
                                                                    margin: [-530,10,0,0],
                                                                    alignment: 'right',  
                                                                    image: 'data:image/jpeg;base64,',
                                                                    width: 50,
                                                                    height: 50
                                                                    */
                                                                        //Tecknicos
                                                                    
                                                                    alignment: 'right',
                                                                    margin: [-420,10,0,0],
                                                                    image: 'data:image/jpeg;base64,',                                                                    
                                                                    width: 160,
                                                                    height: 50
                                                                  }, {
                                                                    margin: [-10,10],
                                                                    alignment: 'left',
                                                                    text: hoyFecha(),
                                                                    fontSize: 10,
                                                                    }
                                                            ]
                                                        }
                                             
                                                    });

                                
                                                       
                                doc.content.splice(0, 0,                          
                                  {
                                    alignment: 'left',
                                    text: 'Reporte de datos FMS 1 ',
                                    fontSize: 25
                                  },
                                  {
                                    alignment: 'left',
                                    text: $.i18n('title_period') + ':' + $("#from").val() + " Hasta " + $("#to").val(),
                                    fontSize: 10
                                  },
                                  
                                  {
                                    alignment: 'left',
                                    text: '\n\n',
                                    fontSize: 10
                                  });

                               /* doc.content.push(
                                    {   
                                        margin: [1,15,1,1],
                                        alignment: 'center',
                                        image: imageData                                    
                                    }
                                );*/
                            }
                        }, {
                    extend: 'print',
                    text: $.i18n('button_print'),
                    title: $.i18n("title_report_fms1") + "",
                    
                       customize: function(win) {
                            var header = '<span style="position:absolute;top:53px;left:0;">'+$.i18n('title_period')+':'+$("#from").val() + " => " + $("#to").val()+'</span>'+
                                '<img src="' + sessionStorage.getItem('logomark') + '" style="position:absolute; top:0; right:0;" />';

                            $(win.document.body)
                                .css( 'font-size', '10pt' )
                                .css( 'background-color', '#fff' )
                                .prepend(
                                    header
                                );
         
                            $(win.document.body).find( 'table' )
                            .addClass( 'compact' )
                            .css( 'font-size', 'inherit' );
                            
                             
                    }
                }]
            });

            doOpenAlertSucess($.i18n("title_success"), $.i18n("title_message_generated"), 1500);

        }

    });


}


function doPrepareEvents2() {

    doOpenAlertWait($.i18n("title_wait"), $.i18n("title_wait_report"));

    var form = $("#form_report2").serializeObject();
    var params = "";
    var parameters = "";
    //var params = new Object();
    //prepare list devices
    var deviceIds = $('#deviceId2 option:selected');
    $(deviceIds).each(function(index, deviceId) {
        if ($(this).val() != "") {
            params += "deviceId=" + $(this).val() + "&";
        }
    });

    //prepare list groups

    var groups = $('#groupId option:selected');
    $(groups).each(function(index, group) {
        if ($(this).val() != "") {
            params += "groupId=" + $(this).val() + "&";
        }
    });
    //params += "groupId=0&";

    //prepare list type events

    var types = $('#type option:selected');
    $(types).each(function(index, type) {
        if ($(this).val() != "") {
            parameters += $(this).val() + "|";
        }
    });
    //params += "type=alarm&";

    var data_ini = moment(form.from2, "DD/MM/YYYY hh:mm:ss");
    var data_end = moment(form.to2, "DD/MM/YYYY hh:mm:ss");

    params += "from=" + data_ini.tz("Etc/GMT+0").format().substr(0, 19) + ".000Z&";
    params += "to=" + data_end.tz("Etc/GMT+0").format().substr(0, 19) + ".000Z";

    console.log(sessionStorage.getItem('url') + "positions")

    $.ajax({
        type: "GET",
        url: sessionStorage.getItem('url') + "reports/route",
        data: params,
        contentType: "application/json",
        cache: false,
        headers: {
            "Authorization": "Basic " + btoa(sessionStorage.getItem('email') + ":" + sessionStorage.getItem('password')),
            "Accept": "application/json"
        },
        error: function(response) {
            switch (response.status) {
                case 400:
                    doOpenAlertError($.i18n("message_user_not_permission"));
                    break;
                case 401:
                    doOpenAlertError($.i18n("message_user_unauthorized"));
                    break;
                case 404:
                    doOpenAlertError($.i18n("message_user_no_matching"));
                    break;
                default:
                    doOpenAlertError($.i18n("message_error_performing"));
                    break;
            }
        },
        success: function(response) {

            // Javascript sourced data
            dataSet.splice(0);

            // var alarm = ("attributes" in value.attributes?value.attributes.attributes:'');
            // dataSet.push([moment(value.serverTime).format('DD/MM/YYYY HH:mm:ss'), findDevice(value.deviceId).name, findEventType(value.type).name, attributes, (value.geofenceId>0?findGeofence(value.geofenceId).name:"")]);
            //console.log(parameters)
            //parameters.indexOf(alarm);
            //j = parameters.search("allEvent")
            //console.log('j=' + j)
            //if (j != -1) {
            //    parameters = "obd"
            //}
            console.log('Entro al success')
            var tempRpm =[];
            var tempTemper = [];
            var tempCombus = [];
            var tempTime = [];
            $.each(response, function(key, value) {
                var obd = ('Tipo' in value.attributes) ? value.attributes.Tipo : "";
                //console.log('el artibuto=' + obd)

                
                  if (obd == "FMS2") {
                   // console.log('los rpm=' + value.attributes.rpm)

                    var telltale_block = ('fms1_telltale_current_block' in value.attributes) ? value.attributes.fms1_telltale_current_block : "";
                    var telltale1 = ('fms1_telltale_status_1' in value.attributes) ? value.attributes.fms1_telltale_status_1 : "";
                    var telltale2 = ('fms1_telltale_status_2' in value.attributes) ? value.attributes.fms1_telltale_status_2 : "";
                    var telltale3 = ('fms1_telltale_status_3' in value.attributes) ? value.attributes.fms1_telltale_status_3 : "";
                    var telltale4 = ('fms1_telltale_status_4' in value.attributes) ? value.attributes.fms1_telltale_status_4 : "";
                    var telltale5 = ('fms1_telltale_status_5' in value.attributes) ? value.attributes.fms1_telltale_status_5 : "";
                    var telltale6 = ('fms1_telltale_status_6' in value.attributes) ? value.attributes.fms1_telltale_status_6 : "";
                    var telltale7 = ('fms1_telltale_status_7' in value.attributes) ? value.attributes.fms1_telltale_status_7 : "";
                    var telltale8 = ('fms1_telltale_status_8' in value.attributes) ? value.attributes.fms1_telltale_status_8 : "";
                    telltale_block = traduce1(telltale_block);
                    telltale1 = traduce1(telltale1);
                    telltale2 = traduce1(telltale2);
                    telltale3 = traduce1(telltale3);
                    telltale4 = traduce1(telltale4);
                    telltale5 = traduce1(telltale5);
                    telltale6 = traduce1(telltale6);
                    telltale7 = traduce1(telltale7);
                    telltale8 = traduce1(telltale8);
                    // dataSet.push([moment(value.serverTime).format('DD/MM/YYYY HH:mm:ss'), findDevice(value.deviceId).name, traduce(alarm), value.speed]);
                    dataSet.push([moment(value.serverTime).format('DD/MM/YYYY HH:mm:ss'),
                    findDevice(value.deviceId).name,
                    telltale_block,
                    telltale1,
                    telltale2,
                    telltale3,
                    telltale4,
                    telltale5,
                    telltale6,
                    telltale7,
                    telltale8]);
                    //tempRpm.push(rpm);
                    //tempTemper.push(coolantTemp);
                    //tempCombus.push(fuel);
                    //tempTime.push(moment(value.serverTime).format('DD/MM/YYYY HH:mm:ss'));                    
                }
                
                
                
            });

            //grafica(tempRpm, tempTemper, tempCombus, tempTime);

            $('#datatable2').dataTable().fnDestroy();

            $('#datatable2').dataTable({
                data: dataSet,
                columnDefs: [],
                responsive: true,
                dom: 'Blfrtip',
                colReorder: true,
                buttons: [{
                
                //{extend: 'excelHtml5',
			              // title: $.i18n("title_reports_events")
			              //}
                          
                           extend: 'pdfHtml5',
                            title: $.i18n("title_report_fms1"),
                            className: 'expPDF',                    
                            customize: function(doc) {
                            doc['pageMargins'] = [10, 70, 10, 10];
                            //doc.pageMargins = [10,10,10,10];
                            doc['header'] = (function (page, pages) {
                                                        return {
                                                            columns: [
                                                               {
                                                                    //Milipol
                                                                    /*
                                                                    margin: [-530,10,0,0],
                                                                    alignment: 'right',  
                                                                    image: 'data:image/jpeg;base64,',
                                                                    width: 50,
                                                                    height: 50
                                                                    */
                                                                        //Tecknicos
                                                                    
                                                                    alignment: 'right',
                                                                    margin: [-420,10,0,0],
                                                                    image: 'data:image/jpeg;base64,',                                                                    
                                                                    width: 160,
                                                                    height: 50
                                                                  }, {
                                                                    margin: [-10,10],
                                                                    alignment: 'left',
                                                                    text: hoyFecha(),
                                                                    fontSize: 10,
                                                                    }
                                                            ]
                                                        }
                                             
                                                    });

                                
                                                       
                                doc.content.splice(0, 0,                          
                                  {
                                    alignment: 'left',
                                    text: 'Reporte de datos FMS 2 ',
                                    fontSize: 25
                                  },
                                  {
                                    alignment: 'left',
                                    text: $.i18n('title_period') + ':' + $("#from2").val() + " Hasta " + $("#to2").val(),
                                    fontSize: 10
                                  },
                                  
                                  {
                                    alignment: 'left',
                                    text: '\n\n',
                                    fontSize: 10
                                  });

                                /*doc.content.push(
                                    {   
                                        margin: [1,15,1,1],
                                        alignment: 'center',
                                        image: imageData                                    
                                    }
                                );*/
                            }
                        }, {
                    extend: 'print',
                    text: $.i18n('button_print'),
                    title: $.i18n("title_report_fms1") + "",
                    
                       customize: function(win) {
                            var header = '<span style="position:absolute;top:53px;left:0;">'+$.i18n('title_period')+':'+$("#from2").val() + " => " + $("#to2").val()+'</span>'+
                                '<img src="' + sessionStorage.getItem('logomark') + '" style="position:absolute; top:0; right:0;" />';

                            $(win.document.body)
                                .css( 'font-size', '10pt' )
                                .css( 'background-color', '#fff' )
                                .prepend(
                                    header
                                );
         
                            $(win.document.body).find( 'table' )
                            .addClass( 'compact' )
                            .css( 'font-size', 'inherit' );
                            
                              /* $(win.document.body)
                            .append(                                
                                '<img src="'+imageData+'" style="position:static; right: 0%; width: 100%; height:90%; " />'
                            );*/
                    }
                }]
            });

            doOpenAlertSucess($.i18n("title_success"), $.i18n("title_message_generated"), 1500);

        }

    });


}



function doPrepareEvents3() {

    doOpenAlertWait($.i18n("title_wait"), $.i18n("title_wait_report"));

    var form = $("#form_report3").serializeObject();
    var params = "";
    var parameters = "";
    //var params = new Object();
    //prepare list devices
    var deviceIds = $('#deviceId3 option:selected');
    $(deviceIds).each(function(index, deviceId) {
        if ($(this).val() != "") {
            params += "deviceId=" + $(this).val() + "&";
        }
    });

    //prepare list groups

    var groups = $('#groupId option:selected');
    $(groups).each(function(index, group) {
        if ($(this).val() != "") {
            params += "groupId=" + $(this).val() + "&";
        }
    });
    //params += "groupId=0&";

    //prepare list type events

    var types = $('#type option:selected');
    $(types).each(function(index, type) {
        if ($(this).val() != "") {
            parameters += $(this).val() + "|";
        }
    });
    //params += "type=alarm&";

    var data_ini = moment(form.from3, "DD/MM/YYYY hh:mm:ss");
    var data_end = moment(form.to3, "DD/MM/YYYY hh:mm:ss");

    params += "from=" + data_ini.tz("Etc/GMT+0").format().substr(0, 19) + ".000Z&";
    params += "to=" + data_end.tz("Etc/GMT+0").format().substr(0, 19) + ".000Z";

    console.log(sessionStorage.getItem('url') + "positions")

    $.ajax({
        type: "GET",
        url: sessionStorage.getItem('url') + "reports/route",
        data: params,
        contentType: "application/json",
        cache: false,
        headers: {
            "Authorization": "Basic " + btoa(sessionStorage.getItem('email') + ":" + sessionStorage.getItem('password')),
            "Accept": "application/json"
        },
        error: function(response) {
            switch (response.status) {
                case 400:
                    doOpenAlertError($.i18n("message_user_not_permission"));
                    break;
                case 401:
                    doOpenAlertError($.i18n("message_user_unauthorized"));
                    break;
                case 404:
                    doOpenAlertError($.i18n("message_user_no_matching"));
                    break;
                default:
                    doOpenAlertError($.i18n("message_error_performing"));
                    break;
            }
        },
        success: function(response) {

            // Javascript sourced data
            dataSet.splice(0);

            // var alarm = ("attributes" in value.attributes?value.attributes.attributes:'');
            // dataSet.push([moment(value.serverTime).format('DD/MM/YYYY HH:mm:ss'), findDevice(value.deviceId).name, findEventType(value.type).name, attributes, (value.geofenceId>0?findGeofence(value.geofenceId).name:"")]);
            //console.log(parameters)
            //parameters.indexOf(alarm);
            //j = parameters.search("allEvent")
            //console.log('j=' + j)
            //if (j != -1) {
            //    parameters = "obd"
            //}
            console.log('Entro al success')
            var tempRpm =[];
            var tempTemper = [];
            var tempCombus = [];
            var tempTime = [];
            $.each(response, function(key, value) {
                var obd = ('Tipo' in value.attributes) ? value.attributes.Tipo : "";
                //console.log('el artibuto=' + obd)

                
                  if (obd == "FMS3") {
                   // console.log('los rpm=' + value.attributes.rpm)

                    //var posi_door = ('dc1_position_of_doors' in value.attributes) ? value.attributes.dc1_position_of_doors : "";
                    //var ramp_chair = ('dc1_ramp_chair_lift' in value.attributes) ? value.attributes.dc1_ramp_chair_lift : "";
                    //var status2_door = ('dc1_status2_of_doors' in value.attributes) ? value.attributes.dc1_status2_of_doors : "";
                    
                    
                    var lock_status_door1 = ('dc2_lock_status_door1' in value.attributes) ? value.attributes.dc2_lock_status_door1 : "";
                    var open_status_door1 = ('dc2_open_status_door1' in value.attributes) ? value.attributes.dc2_open_status_door1 : "";
                    var enable_status_door1 = ('dc2_enable_status_door1' in value.attributes) ? value.attributes.dc2_enable_status_door1 : "";
                    
                    var lock_status_door2 = ('dc2_lock_status_door2' in value.attributes) ? value.attributes.dc2_lock_status_door2 : "";
                    var open_status_door2 = ('dc2_open_status_door2' in value.attributes) ? value.attributes.dc2_open_status_door2 : "";
                    var enable_status_door2 = ('dc2_enable_status_door2' in value.attributes) ? value.attributes.dc2_enable_status_door2 : "";
                    
                    var lock_status_door3 = ('dc2_lock_status_door3' in value.attributes) ? value.attributes.dc2_lock_status_door3 : "";
                    var open_status_door3 = ('dc2_open_status_door3' in value.attributes) ? value.attributes.dc2_open_status_door3 : "";
                    var enable_status_door3 = ('dc2_enable_status_door3' in value.attributes) ? value.attributes.dc2_enable_status_door3 : "";
                    
                    
                    var lock_status_door4 = ('dc2_lock_status_door4' in value.attributes) ? value.attributes.dc2_lock_status_door4 : "";
                    var open_status_door4 = ('dc2_open_status_door4' in value.attributes) ? value.attributes.dc2_open_status_door4 : "";
                    var enable_status_door4 = ('dc2_enable_status_door4' in value.attributes) ? value.attributes.dc2_enable_status_door4 : "";
                    
                    var lock_status_door5 = ('dc2_lock_status_door5' in value.attributes) ? value.attributes.dc2_lock_status_door5 : "";
                    var open_status_door5 = ('dc2_open_status_door5' in value.attributes) ? value.attributes.dc2_open_status_door5 : "";
                    var enable_status_door5 = ('dc2_enable_status_door5' in value.attributes) ? value.attributes.dc2_enable_status_door5 : "";
                    
                    var lock_status_door6 = ('dc2_lock_status_door6' in value.attributes) ? value.attributes.dc2_lock_status_door6 : "";
                    var open_status_door6 = ('dc2_open_status_door6' in value.attributes) ? value.attributes.dc2_open_status_door6 : "";
                    var enable_status_door6 = ('dc2_enable_status_door6' in value.attributes) ? value.attributes.dc2_enable_status_door6 : "";
                    
                    var lock_status_door7 = ('dc2_lock_status_door7' in value.attributes) ? value.attributes.dc2_lock_status_door7 : "";
                    var open_status_door7 = ('dc2_open_status_door7' in value.attributes) ? value.attributes.dc2_open_status_door7 : "";
                    var enable_status_door7 = ('dc2_enable_status_door7' in value.attributes) ? value.attributes.dc2_enable_status_door7 : "";
                    
                    var lock_status_door8 = ('dc2_lock_status_door8' in value.attributes) ? value.attributes.dc2_lock_status_door8 : "";
                    var open_status_door8 = ('dc2_open_status_door8' in value.attributes) ? value.attributes.dc2_open_status_door8 : "";
                    var enable_status_door8 = ('dc2_enable_status_door8' in value.attributes) ? value.attributes.dc2_enable_status_door8 : "";
                    
                    var lock_status_door9 = ('dc2_lock_status_door9' in value.attributes) ? value.attributes.dc2_lock_status_door9 : "";
                    var open_status_door9 = ('dc2_open_status_door9' in value.attributes) ? value.attributes.dc2_open_status_door9 : "";
                    var enable_status_door9 = ('dc2_enable_status_door9' in value.attributes) ? value.attributes.dc2_enable_status_door9 : "";
                    
                    var lock_status_door10 = ('dc2_lock_status_door10' in value.attributes) ? value.attributes.dc2_lock_status_door10 : "";
                    var open_status_door10 = ('dc2_open_status_door10' in value.attributes) ? value.attributes.dc2_open_status_door10 : "";
                    var enable_status_door10 = ('dc2_enable_status_door10' in value.attributes) ? value.attributes.dc2_enable_status_door10 : "";
                    
                    var alternator1 = ('as_alternator_status1' in value.attributes) ? value.attributes.as_alternator_status1 : "";
                    var alternator2 = ('as_alternator_status2' in value.attributes) ? value.attributes.as_alternator_status2 : "";
                    var alternator3 = ('as_alternator_status3' in value.attributes) ? value.attributes.as_alternator_status3 : "";
                    var alternator4 = ('as_alternator_status4' in value.attributes) ? value.attributes.as_alternator_status4 : "";
                    
                      lock_status_door1 = traduce(lock_status_door1);
                      open_status_door1 = traduce(open_status_door1);
                      enable_status_door1 = traduce(enable_status_door1);
                    
                      lock_status_door2 = traduce(lock_status_door2);
                      open_status_door2 = traduce(open_status_door2);
                      enable_status_door2 = traduce(enable_status_door2);
                      
                      lock_status_door3 = traduce(lock_status_door3);
                      open_status_door3 = traduce(open_status_door3);
                      enable_status_door3 = traduce(enable_status_door3);
                      
                      lock_status_door4 = traduce(lock_status_door4);
                      open_status_door4 = traduce(open_status_door4);
                      enable_status_door4 = traduce(enable_status_door4);
                      
                      lock_status_door5 = traduce(lock_status_door5);
                      open_status_door5 = traduce(open_status_door5);
                      enable_status_door5 = traduce(enable_status_door5);
                      
                      lock_status_door6 = traduce(lock_status_door6);
                      open_status_door6 = traduce(open_status_door6);
                      enable_status_door6 = traduce(enable_status_door6);
                      
                      lock_status_door7 = traduce(lock_status_door7);
                      open_status_door7 = traduce(open_status_door7);
                      enable_status_door7 = traduce(enable_status_door7);
                      
                      lock_status_door8 = traduce(lock_status_door8);
                      open_status_door8 = traduce(open_status_door8);
                      enable_status_door8 = traduce(enable_status_door8);
                      
                      lock_status_door9 = traduce(lock_status_door9);
                      open_status_door9 = traduce(open_status_door9);
                      enable_status_door9 = traduce(enable_status_door9);
                      
                      lock_status_door10 = traduce(lock_status_door10);
                      open_status_door10 = traduce(open_status_door10);
                      enable_status_door10 = traduce(enable_status_door10);
                      
              
                   
                    // dataSet.push([moment(value.serverTime).format('DD/MM/YYYY HH:mm:ss'), findDevice(value.deviceId).name, traduce(alarm), value.speed]);
                    dataSet.push([moment(value.serverTime).format('DD/MM/YYYY HH:mm:ss'), 
                    findDevice(value.deviceId).name, 
                    lock_status_door1 + " " + " " + open_status_door1 + " " + " " + enable_status_door1,
                     
                    lock_status_door2 + " " + " " + open_status_door2 + " " + " " + enable_status_door2,
                    
                    lock_status_door3 + " " + " " + open_status_door3 + " " + " " + enable_status_door3,
                     
                    lock_status_door4 + " " + " " + open_status_door4 + " " + " " + enable_status_door4,
                    
                    lock_status_door5 + " " + " " + open_status_door5 + " " + " " + enable_status_door5
                    
                    //lock_status_door6 + " " + open_status_door6 + " "+ enable_status_door6,
                    
                    //lock_status_door7 + " " + open_status_door7 + " "+ enable_status_door7,
                    
                    //lock_status_door8 + " " + open_status_door8 + " "+ enable_status_door8,
                    
                    //lock_status_door9 + " " + open_status_door9 + " "+ enable_status_door9,
                    
                   // lock_status_door10 + " " + open_status_door10 +" "+ enable_status_door10
                    ]);
                    //tempRpm.push(rpm);
                    //tempTemper.push(coolantTemp);
                    //tempCombus.push(fuel);
                    //tempTime.push(moment(value.serverTime).format('DD/MM/YYYY HH:mm:ss'));                    
                }
                
                
                
            });

            //grafica(tempRpm, tempTemper, tempCombus, tempTime);

            $('#datatable3').dataTable().fnDestroy();

            $('#datatable3').dataTable({
                data: dataSet,
                columnDefs: [],
                responsive: true,
                dom: 'Blfrtip',
                colReorder: true,
                buttons: [{
                
                //{extend: 'excelHtml5',
			              // title: $.i18n("title_reports_events")
			              //}
                          
                           extend: 'pdfHtml5',
                            title: $.i18n("title_report_fms1"),
                             exportOptions: {
                        columns: [0, 1, 2, 3, 4, 5, 6]
                    },                            
                            className: 'expPDF',                    
                            customize: function(doc) {
                            doc['pageMargins'] = [10, 70, 10, 10];
                            //doc.pageMargins = [10,10,10,10];
                            doc['header'] = (function (page, pages) {
                                                        return {
                                                            columns: [
                                                               {
                                                                    //Milipol
                                                                    /*
                                                                    margin: [-530,10,0,0],
                                                                    alignment: 'right',  
                                                                    image: 'data:image/jpeg;base64,',
                                                                    width: 50,
                                                                    height: 50
                                                                    */
                                                                        //Tecknicos
                                                                    
                                                                    alignment: 'right',
                                                                    margin: [-420,10,0,0],
                                                                    image: 'data:image/jpeg;base64,',                                                                    
                                                                    width: 160,
                                                                    height: 50
                                                                  }, {
                                                                    margin: [-10,10],
                                                                    alignment: 'left',
                                                                    text: hoyFecha(),
                                                                    fontSize: 10,
                                                                    }
                                                            ]
                                                        }
                                             
                                                    });

                                
                                                       
                                doc.content.splice(0, 0,                          
                                  {
                                    alignment: 'left',
                                    text: 'Reporte de datos FMS 3 ',
                                    fontSize: 25
                                  },
                                  {
                                    alignment: 'left',
                                    text: $.i18n('title_period') + ':' + $("#from3").val() + " Hasta " + $("#to3").val(),
                                    fontSize: 10
                                  },
                                  
                                  {
                                    alignment: 'left',
                                    text: '\n\n',
                                    fontSize: 10
                                  });

                                /*doc.content.push(
                                    {   
                                        margin: [1,15,1,1],
                                        alignment: 'center',
                                        image: imageData                                    
                                    }
                                );*/
                            }
                        }, {
                    extend: 'print',
                    text: $.i18n('button_print'),
                    title: $.i18n("title_report_fms1") + "",
                    
                       customize: function(win) {
                            var header = '<span style="position:absolute;top:53px;left:0;">'+$.i18n('title_period')+':'+$("#from3").val() + " => " + $("#to3").val()+'</span>'+
                                '<img src="' + sessionStorage.getItem('logomark') + '" style="position:absolute; top:0; right:0;" />';

                            $(win.document.body)
                                .css( 'font-size', '10pt' )
                                .css( 'background-color', '#fff' )
                                .prepend(
                                    header
                                );
         
                            $(win.document.body).find( 'table' )
                            .addClass( 'compact' )
                            .css( 'font-size', 'inherit' );
                            
                              /* $(win.document.body)
                            .append(                                
                                '<img src="'+imageData+'" style="position:static; right: 0%; width: 100%; height:90%; " />'
                            );*/
                    },
                    exportOptions: {
                       // quite la columna de imagen del reporte  columns: [0, 1, 2, 3, 4, 5, 6, 7]
                        columns: [0, 1, 2, 3, 4, 5, 6]
                    }
                }]
            });

            doOpenAlertSucess($.i18n("title_success"), $.i18n("title_message_generated"), 1500);

        }

    });


}

function traduce(alarma) {
    switch (alarma) {

        case '0':
            final = 'NO'
            break;
        case '1':
            final = 'SI'
            break;
        default:
            final = alarma
    }
    return final

}

function traduce1(alarma1) {
    switch (alarma1) {

        case '0':
            final = 'No activo'
            break;
        case '1':
            final = 'Activo'
            break;
        default:
            final = alarma1
    }
    return final

}


/**
 * Carrega lista dos devices
 * 
 
  <option value="allEvents" data-i18n="title_allEvents">All Events</option>												                    
					                    <option value="MobileyeFcw" >Advertencia de Colisi&oacute;n Frontal. FCW</option>
					                    <option value="MobileyePcw" >Advertencia de Colisi&oacute;n a Peatones. PCW</option>
					                    <option value="MobileyeUfcw" >Advertencia Urbana de Colisi&oacute;n Frontal por debajo de 30 km/h. UFCW</option>
					                    <option value="MobileyeTamper" >Mobileye Tamper Activado</option>
					                    <option value="MobileyeError" >Mobileye Error</option>
					                    <option value="MobileyeTsr" >Indicador de L&iacute;mite de Velocidad Excedida. SLI</option>
					                    <option value="MobileyeHmw" >Advertencia de Distancia Segura Frontal. HMW</option>
					                    <option value="MobileyeLl" >Detector de l&iacute;neas Viales Izquierda LD</option>
					                    <option value="MobileyeLr" >Detector de L&iacute;neas Viales Derecha LDW</option>
					             					                    
 * 
 * @return void
 */

function getDevicesEvents() {

    $.ajax({
        type: "get",
        url: sessionStorage.getItem('url') + "devices/",
        cache: false,
        headers: {
            "Authorization": "Basic " + btoa(sessionStorage.getItem('email') + ":" + sessionStorage.getItem('password'))
        },
        error: function(response) {
            switch (response.status) {
                case 400:
                    doOpenAlertError($.i18n("message_user_not_permission"));
                    break;
                case 401:
                    doOpenAlertError($.i18n("message_user_unauthorized"));
                    break;
                default:
                    doOpenAlertError($.i18n("message_error_performing"));
                    break;
            }
        },
        success: function(response) {

            limpaSelect("deviceId");
            devices.splice(0);

            $.each(response, function(key, value) {
                insertOption("deviceId", value.id, value.name);

                devices.push({
                    deviceId: value.id,
                    name: value.name,
                    category: value.category,
                    uniqueId: value.uniqueId
                });
            });

        }
    });

}


function getDevicesEvents2() {

    $.ajax({
        type: "get",
        url: sessionStorage.getItem('url') + "devices/",
        cache: false,
        headers: {
            "Authorization": "Basic " + btoa(sessionStorage.getItem('email') + ":" + sessionStorage.getItem('password'))
        },
        error: function(response) {
            switch (response.status) {
                case 400:
                    doOpenAlertError($.i18n("message_user_not_permission"));
                    break;
                case 401:
                    doOpenAlertError($.i18n("message_user_unauthorized"));
                    break;
                default:
                    doOpenAlertError($.i18n("message_error_performing"));
                    break;
            }
        },
        success: function(response) {

            limpaSelect("deviceId2");
            devices.splice(0);

            $.each(response, function(key, value) {
                insertOption("deviceId2", value.id, value.name);

                devices.push({
                    deviceId: value.id,
                    name: value.name,
                    category: value.category,
                    uniqueId: value.uniqueId
                });
            });

        }
    });

}



function getDevicesEvents3() {

    $.ajax({
        type: "get",
        url: sessionStorage.getItem('url') + "devices/",
        cache: false,
        headers: {
            "Authorization": "Basic " + btoa(sessionStorage.getItem('email') + ":" + sessionStorage.getItem('password'))
        },
        error: function(response) {
            switch (response.status) {
                case 400:
                    doOpenAlertError($.i18n("message_user_not_permission"));
                    break;
                case 401:
                    doOpenAlertError($.i18n("message_user_unauthorized"));
                    break;
                default:
                    doOpenAlertError($.i18n("message_error_performing"));
                    break;
            }
        },
        success: function(response) {

            limpaSelect("deviceId3");
            devices.splice(0);

            $.each(response, function(key, value) {
                insertOption("deviceId3", value.id, value.name);

                devices.push({
                    deviceId: value.id,
                    name: value.name,
                    category: value.category,
                    uniqueId: value.uniqueId
                });
            });

        }
    });

}


function localFormClear() {
    doFormClear();

    $("#deviceId").select2("val", "");
    $("#groupId").select2("val", "");
    $("#type").select2("val", "");

    if (dataSet.length > 0) { $('.datatable-js').dataTable().fnDestroy(); }

    dataSet.splice(0);

    $('.datatable-js').dataTable({
        data: dataSet,
        columnDefs: [],
        dom: 'Bfrtip',
        buttons: []
    });

}


function hoyFecha(){
    var hoy = new Date();
        var dd = hoy.getDate();
        var mm = hoy.getMonth()+1;
        var yyyy = hoy.getFullYear();
        
        dd = addZero(dd);
        mm = addZero(mm);
 
        return dd+'/'+mm+'/'+yyyy;
}

function addZero(i) {
    if (i < 10) {
        i = '0' + i;
    }
    return i;
}

function findPositionId(tempPosition, id) {
    return _.find(tempPosition, function (obj) {
        //console.log(dump(obj));
        return obj.id == id;
    }) || {};
}

function grafica(itemsRpm, itemsTemp, itemsComb, itemsTime){

    var options = {
    chart: {
        type: 'line'
    },
    title: {
        text: 'Reporte de eventos OBDII'
    },
    subtitle: {
        text: 'Grafica de eventos'
    },
    xAxis: {
        categories: itemsTime
    },
    yAxis: {
        title: {
            text: 'Grafico de eventos'
        }
    },
    plotOptions: {
        line: {
            dataLabels: {
                enabled: true
            },
            enableMouseTracking: false
        }
    },
    series: [{
        name: 'RPM',
        data: itemsRpm,
        dataLabels: {
            format: '{point.y:1f}'+' rpm'
        }
        
    }, {
        name: 'Tempetura',
        data: itemsTemp,
        dataLabels: {
            format: '{point.y:1f}'+' °C'
        }        
    }, {
        name: 'Combustible',
        data: itemsComb,
        dataLabels: {
            format: '{point.y:1f}'+' %'
        }        
    }
    ]
};

char = Highcharts.chart('container', options);

var data = {
    options: JSON.stringify(options),
    filename: 'test.png',
    type: 'image/png',
    async: true
};

var exportUrl = 'https://export.highcharts.com/';
$.post(exportUrl, data, function(data) {
    var imageUrl = exportUrl + data;
    var urlCreator = window.URL || window.webkitURL;
    toDataURL(imageUrl, function(dataUrl) {            
        imageData = dataUrl;
    });
    //console.log(imageUrl);
    fetch(imageUrl).then(response => response.blob()).then(data => {console.log(data)});
});

function toDataURL(url, callback) {
  var xhr = new XMLHttpRequest();
  xhr.onload = function() {
    var reader = new FileReader();
    reader.onloadend = function() {
      callback(reader.result);
    }
    reader.readAsDataURL(xhr.response);
  };
  xhr.open('GET', url);
  xhr.responseType = 'blob';
  xhr.send();
}

}
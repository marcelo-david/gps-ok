        ...
        success: function(response) {
            ...
            $.each(response, function(key, value) {
                ...
                if (obd == "OBD") {
                    ...
                    var rpm = ('rpm' in value.attributes) ? value.attributes.rpm : "";
                    var vss = ('VSS' in value.attributes) ? value.attributes.vss : "";
                    var fuel = ('fuel' in value.attributes) ? value.attributes.fuel : "";
                    var coolantTemp = ('coolantTemp' in value.attributes) ? value.attributes.coolantTemp : "";
                    // dataSet.push([moment(value.serverTime).format('DD/MM/YYYY HH:mm:ss'), findDevice(value.deviceId).name, traduce(alarm), value.speed]);
                    dataSet.push([moment(value.serverTime).format('DD/MM/YYYY HH:mm:ss'), findDevice(value.deviceId).name, rpm+" rpm", vss, fuel+" %",  coolantTemp+" C"]);
                    tempRpm.push(rpm);
